...
export function addRentals(data) {
  return (dispatch) => {
    return apiClient
      .post(`${endpoints().bikeRentalsAPI}`, data)
      .then((response) => {
        dispatch(
          fetchList(
            "bikeRentals",
            `${endpoints().bikeRentalsAPI}/search/rented`,
            1,
            10
          )
        );
...
export function deleteRentals(id, status) {
  return (dispatch) => {
    return apiClient
      .delete(`${endpoints().bikeRentalsAPI}/${id}`)
      .then((response) => {
        dispatch(
          fetchList(
            "bikeRentals",
            `${endpoints().bikeRentalsAPI}/search/${status}`,
            1,
            10
          )
        );
...
export function closeRentals(data) {
  return (dispatch) => {
    return apiClient
      .put(`${endpoints().bikeRentalsAPI}/close`, data)
      .then((response) => {
        let successMessage;
        if (response && response.data) {
          successMessage = response.data.message;
...
export function updateRentals(data) {
  return (dispatch) => {
    return apiClient
      .put(`${endpoints().bikeRentalsAPI}`, data)
      .then((response) => {
        dispatch(
          fetchList(
            "bikeRentals",
